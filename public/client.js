var socket = io('/' + window.location.host + window.location.pathname); // Ports not working because the relative url isn't detecting the port number, could change it to full URL, but it's probably fixed in a new Socket.IO version.
...
var radarViewRadius = 1000;

function pinPoint() {
    rpctx.clearRect(0, 0, 60, 60);
    for (var n in ONLINE.players) {
        if (ONLINE.players[n].nick !== player.info.nick) {
            var x = ((ONLINE.players[n].x - player.info.x) / radarViewRadius) * 60;
            var y = ((ONLINE.players[n].y - player.info.y) / radarViewRadius) * 60;
